...
> 최종 갱신: 2026-07-31 (연한글씨 수정 + T4/T5/T7 완료 — 새 파이프라인 F1@0.3 0.891, 상세 `DEVLOG.md` 12막)
...
> ⚠️ **2026-07-27~31 전처리를 측지 재구성 + 연한글씨 보존 라우팅으로 재작성**(아래 07-27·07-30~31 항목 /
> `DEVLOG.md` 11·12막)하면서 전처리·좌표계가 바뀌었습니다. **위 옛(adaptiveThreshold) F1(0.968/0.938)은 폐기.**
> **새 파이프라인 F1@0.3 0.891 / @0.5 0.820**(12장, T5·T7 완료)이 현재 값 — `STATUS.md` §1.
...
- **종합점수** = 지표별 + 교육적 가중(정렬·균일 3:3:3:2:2), **명료도는 경고만(점수 제외)**.
...
- (07-27 시점 남은 것 T4·T5·T7·연한글씨는 아래 07-30~31 항목에서 모두 완료됨.)

### 2026-07-30~31 — 연한글씨 수정 + T4·T5·T7 완료, 새 파이프라인 F1 측정 (상세: `DEVLOG.md` 12막)

1. **🐛 연한 글씨 손실 수정(커밋 `1fc09cb`)** — 측지 재구성이 seed 없는 연한 획을 비침처럼 지우던 버그
   (test7·test5·data/). **이미지별 라우팅** 도입: near-black 진짜 잉크(진한 앵커) 없으면 gentle(연한 글씨 보존),
   있으면 geodesic(비침 제거). 세기·sharpness로는 연한글씨≡비침 구분 불가라 이미지 단위로 갈랐다. 단위 테스트 34개.
2. **T4 분석기 정리(커밋 `b9d7ed5`)** — 획 굵기·글자별 slant 제거, 문장 기울기(행별 중심선 회귀, 전처리 좌표)
   도입, 채점 5지표(3:3:3:2:2).
3. **T5 GT 재라벨 + T7 F1(커밋 `386c71c`)** — 평가셋 재구성: 옛 7장 폐기 → **12장**(사진 test·test2·test5·test6·
   test7 + 홀드아웃 test8·test_line1 + data/ 태블릿 5장). test4·test3_crop 배제. **새 파이프라인 F1@0.3 0.891 /
   @0.5 0.820**. 오류는 과병합(merge) 우세 = 붙여쓴/흘림·소형 밀집(비학습 레버 소진, 개선은 파인튜닝 영역).
4. **후처리 과최적화 육안 확인** — 홀드아웃 on/off 델타 렌더로 "후처리가 새 오류 안 만듦·괘선 유령 없음" 확인.
5. **문서 최신화(07-31)** — 위 반영해 STATUS/HANDOFF/requirement/handwriting_evaluation/IMPLEMENTATION_PLAN 갱신.
...
5-0. **`../CHANGES_2026-08-17.md`**(통합본 루트) — **2026-08-16~17 변경 내역 + 남은 문제**.
   `ab9de5a`(당겨온 시점) 대비 무엇을 고쳤고·무엇이 남았고·무엇을 되돌렸는지, 그리고
   **팀원이 각자 해야 할 일 2가지**(마이그레이션, `.env` 8줄 삭제)가 있다. 사용자 요청으로
   신설한 인수인계 문서 — 이 기간 작업을 넘겨받는 사람은 여기부터 읽으면 된다.
...
│                                        2026-09-02: 항목 5개 재정의 + 글자별 ok/failed_items
│
├── tests/                            ← `python -m pytest ai/tests -q` (git 루트에서)
│   ├── test_scoring.py               ← 이미지 5지표 등급·종합점수
│   ├── test_norm_deviations.py       ← 절대 규범 축(기울기·자간·행간) — ⚠️ '기울기'는 **행 수평 이탈**
│   ├── test_image_item_scoring.py    ← **이미지 채점 5항목 + 글자 단위 초록/빨강 판정** (2026-09-02)
│   ├── test_canvas_item_scoring.py   ← **캔버스 항목 독립성 + 허용치 양쪽 끝 고정** (2026-09-01)
│   ├── test_jamo_layout_contract.py  ← **프론트 dart를 파싱해** 자모 배치·획수·모양·순서 대조
│   ├── test_grouping_expected_count.py / test_solo_jamo_stroke_order.py /
│   │   test_stroke_order_variants.py ← 그룹핑·낱자 획순·복수 정본
│   └── test_preprocessor.py          ← 전처리 (난수 시드 고정)
│
├── canvas/                           ← 캔버스 모드 (아래 4절에서 상세 설명)
│   ├── stroke_grouping.py            ← SFR-004C 규칙 기반 그룹핑 (LSTM 스텁은 2026-09-01 제거)
│   ├── stroke_standards.py           ← SFR-005C 표준 획순 데이터 (LSTM 스텁은 2026-09-01 제거)
│   ├── synthetic_stroke_generator.py ← **자모 배치 정본**(jamo_boxes) + 표준 획 경로.
│   │                                    실측 명조 글리프 기준(2026-09-01). 프론트와 계약 테스트로 고정
│   ├── canvas_quality_analyzer.py    ← SFR-005C 종합 분석 (실제 동작하는 규칙 기반 구현체).
│   │                                    2026-09-01: 항목 6개 + 성분 박스 build_component_boxes()
...
| 채점 항목·문구·박스 (2026-09-02 개편) | ✅ 항목 5개 재정의 + **문구 6문장 고정** + **글자 단위 초록/빨강 박스** (DEVLOG 32막) |

> ⚠️ **2026-09-02에 이미지 채점이 개편됐습니다(DEVLOG 32막).** 아래 표의 "기울기"는
> 이제 **글자들끼리 기울기가 고른가**입니다. 줄이 올라가나 내려가나는 점수에서 빠지고
> **줄 정렬 문구의 방향**으로만 쓰입니다.
>
> **항목 5개** — 크기 균일성 · **기울기 균일성** · **줄 정렬**(수평인가 + 줄에 앉았나) ·
> 자간 · 행간. 화면에는 **항상 6문장**이 나갑니다(종합 1 + 항목 5). 항목 기준은 **80점**
> (= `_band_score`의 '우수' 경계). **수치는 문구에 넣지 않습니다.**
>
> **박스는 기본 초록**, 크기·기울기·줄 정렬 중 **하나라도** 미흡한 글자만 빨강입니다
> (항목별 OR — 종합 점수로 색을 정하지 않습니다). 자간·행간은 글자 하나에 귀속되지 않아
> 박스에 반영하지 않고 **문구로만** 나갑니다.
>
> **글자를 너무 기울여 쓰는 습관**은 균일성과 **별개 축**입니다(`CHAR_SLANT_NORM_DEG = 10°`).
> 전부 똑같이 기울여 쓰면 균일성은 만점이므로 따로 봅니다. **박스는 치지 않습니다** —
> 글씨체 전체의 습관이라 짚을 글자가 없습니다(사용자 결정).
>
> ⚠️ **`slant_consistency_score`는 이름은 그대로인데 의미가 바뀌었습니다** — 대시보드에
> 이전에 쌓인 값과 뜻이 다릅니다. `mean_char_slant`는 **DB에 저장하지 않습니다**(컬럼 없음).
>
> ⚠️ **`NORM_STROKE_RESEARCH.md` ①이 코드와 어긋나 있습니다** — 문서는 세로획 기준이라
> 적었는데 코드·테스트는 행 수평 이탈을 잽니다. 정정 대기(STATUS §1).
...
> ⚠️ **2026-09-01에 채점 체계가 전면 개편됐습니다(DEVLOG 30막).** 아래 4.1~ 절의 서술 중
> "획순/자간/크기 3항목", "LSTM 스텁 유지"는 낡은 내용입니다. 최신 요약은 바로 아래 상자를,
> 상세는 `STATUS.md` §2를 보세요.
>
> **채점 항목이 5개입니다** — 획순 · **획방향** · **성분비율**(초·중·종성 크기 균형) · 크기 · 자간.
> **연습 종류마다 채점되는 항목이 다릅니다**: 자음·모음(낱자)은 3개(획순·획방향·크기),
> 한 글자는 +성분비율, 단어·문장은 +자간. 합산은 **가중 평균**이고 **측정 불가는 0점이 아니라
> 분모에서 빠집니다**.
>
> **LSTM 스텁 2개는 제거됐습니다** — 새 채점이 전부 기하 계산으로 풀리고 학습 데이터도
> 없어서입니다. ⚠️ 단 `requirement.md`의 `REQ-004C-1`·`REQ-005C-3`은 아직 LSTM을 요구하는
> 상태이고, **명세 개정은 팀 결정 대기 중**입니다.
>
> **필압은 제거**(늘 1.0 상수라 신호가 아니었음), **속도는 채점 미반영·수집만** 유지합니다.
>
> 아래 4.1의 "2단계 학습 전략"에서 **1단계(합성 데이터)는 학습용이 아니라 채점기 검증용으로
> 용도가 바뀌었습니다** — "표준대로 그린 입력이 만점인가"를 확인하는 자리입니다. 실제로 이
> 검증이 2026-09-01에 결함 3건(좌표 프레임 불일치·1획 낱자 오판·임계값 미보정)을 잡았습니다.
>
> **2026-09-01 추가 개편(DEVLOG 31막)** — 채점 항목이 **6개**가 됐습니다: 위 5개에
> **기울기**(곧게 그어야 할 획이 15° 초과로 어긋남)가 더해졌고, 획방향은 **역방향(135° 초과)만**
> 봅니다. 바운딩 박스는 음절 → **성분(초·중·종성) 단위**로 내려갔고 색은 **초록/빨강 2색**,
> 판정은 **항목별 OR**입니다. 낱자는 박스를 치지 않고, 문장은 **빨강만** 그립니다.
>
> **자모 배치 정본이 실측 명조 글리프로 교체됐습니다.** 종전에는 합성 획 경로가 기준이었고
> 그 획이 자기 상자를 안 채워(ㄱ은 45%만) 실측과 최대 57% 어긋나 있었습니다. 이제
> `상자 == 잉크`이고, `tests/test_jamo_layout_contract.py`가 **프론트 dart를 파싱해**
> 배치와 **획수·모양·순서**를 함께 고정합니다 — 하루에 자모 정의가 네 번 어긋난 뒤 만든
> 장치입니다(ㄱ·ㄴ 획수, ㅁ 획수, ㄹ 사선, ㅓ·ㅕ 순서, ㅗ↔ㅜ·ㅛ↔ㅠ 뒤바뀜).

캔버스 모드는 `requirement.md`의 SFR-003C(프론트엔드 담당, 이 저장소 `ai/`의 범위 밖) →
SFR-004C(획 그룹핑) → SFR-005C(채점) 흐름입니다. **실사용자 필기 데이터가 전혀 없는 상태**에서
시작했기 때문에 ML(LSTM) 학습은 불가능했고, 대신 **규칙·기하 기반 알고리즘 + 프로그래밍적으로
생성한 표준 데이터**로 구현했습니다.
