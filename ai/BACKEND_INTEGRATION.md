...
> **AI 어시스턴트(Claude 등)로 통합을 진행하는 경우**: 이 문서를 읽히고 "§0 순서대로 진행"을
> 지시하면 됨 — 충돌 해결 명령(§2.1)·환경 명령(§3)·검증 기대값(§7)까지 그대로 실행 가능하게 써뒀다.
...
### 1.1 요구사항(SFR) 원문과 달라진 점 — 라우트·API 설계 시 주의

구현 과정에서 확정된 조정들. 정본: `requirement.md` 상단 "AI 트랙 결정 메모"(32~49행).

| 항목 | SFR 원문 | 실제 구현 |
|---|---|---|
| **레이턴시** | REQ-004I-1 500ms / REQ-005C-4 300ms·자 | CPU 실측 ~2.9초/장 → **하향 개정 or 비동기 or GPU 중 팀 결정**(§8-4). API 타임아웃 설계 주의 |
| **종합점수** | 지표 합산 | **교육 가중 3:3:3:2:2 평균**. **명료도는 점수 제외** — `clarity_warnings` 경고 필드로만 반환 |
| **SFR-005I "표준 서체 대비"** | 표준 자형과 비교 | **자기 일관성(균일성) + 절대 규범**(`norm_deviations` 별도 필드, 점수 미반영). 이미지 모드는 자유 촬영 — **OCR·목표 텍스트 없음** |
| **기울기 평가** | 글자별 지적 | **문장(행) 단위** 평가로 전환 — 개별 글자 지적 문구 없음 |
| **LSTM** (SFR-004C·005C) | 규칙+LSTM 결합 | 실사용자 데이터 없음 → 계약 함수 2개는 **스텁**. 캔버스 실채점은 규칙 기반 `analyze_canvas_writing()`(§4) |
| **REQ-003I-4 재촬영 안내** | 품질 미달 시 재촬영 요구 | 판정 로직은 `preprocess_image_full`에 구현됨 — **라우트 반영은 백엔드 몫**(§8-5) |
| **범위** | (수행계획서) 개인화·실시간 영상·OCR 변환·자유필기 획순 | **이번 범위 밖 확정** — 두 모드 분석 + 대시보드까지 |
...
### 2.1 `ai_adapters.py` 충돌 — 왜 나고, 어떻게 푸나

두 브랜치가 **같은 경로의 같은 파일을 서로 다른 내용으로** 갖고 있어 충돌이 난다:

| | `feature/backend-setup` (받는 쪽) | `feature/ai-setup` (들어오는 쪽) |
|---|---|---|
| `ai_adapters.py` | **스텁**(placeholder) — contour 탐지·개수 비교·입력 그대로 반환 | **실구현** — CRAFT 탐지·AI 전처리·채점을 `ai/` 패키지로 연결 |
| 함수 이름·파라미터·반환 형식 | 동일 (계약: `AI_MODEL_INTERFACE.md`) | 동일 |

원래 계획이 "백엔드는 스텁으로 먼저 개발 → AI 완성 시 **함수 내부만 교체**"였고(계약 문서 명시),
그 교체가 2026-07-18에 일어났다. 따라서:

- **ai-setup 쪽을 통째로 채택하는 것이 정답.** 시그니처가 같으므로 **기존 백엔드 라우트 코드는
  수정 없이 그대로 동작**한다.
- 해결 명령 (backend 작업 브랜치에서 `git merge origin/feature/ai-setup` 중 충돌이 났을 때):

```bash
git checkout --theirs backend/app/services/ai_adapters.py   # "들어오는 쪽"(ai-setup) 채택
git add backend/app/services/ai_adapters.py
git commit          # 병합 커밋 완료
```

- IDE 병합 툴 기준으로는 이 파일 전체에 "Accept Incoming Change"를 적용하는 것과 같다.
  ⚠️ `merge`가 아니라 `rebase` 중이면 ours/theirs 의미가 뒤집히니 위 merge 흐름을 권장.
- `ai/` 폴더는 backend-setup에 없던 신규 파일들이라 충돌 없이 그대로 들어온다.
