...
>   기울기는 `minAreaRect`가 아니라 **세로획 slant**로 이미 교체됨.
...
| **Outputs** | 전처리 완료 이미지 (이진화된 Grayscale PNG, 표준화 해상도 1280×960), 이미지 품질 점수 (0~100), 전처리 메타데이터 (적용된 필터 목록, 감지된 기울기 각도) |
| **Destination** | SFR-004I (CRAFT 기반 Bounding Box 탐지), FastAPI 서버 임시 스토리지 (`/tmp` 디렉터리) |
| **Action** | ① 앱에서 이미지를 Base64 인코딩하여 FastAPI `/api/v1/image/preprocess`로 POST 전송한다. ② 서버가 Base64를 디코딩하여 OpenCV Mat 객체로 변환한다. ③ Grayscale 변환 → Gaussian Blur 노이즈 제거 → Adaptive Thresholding 이진화를 순차 수행한다. ④ Hough Transform으로 기울기를 감지하고 자동 회전 보정한다. ⑤ 이미지를 표준 해상도(1280×960)로 리사이즈한다. ⑥ 품질 점수를 산출하고 40점 미만이면 재촬영을 요청한다. |
...
| **Requirements** | `REQ-004I-1` CRAFT 모델의 단일 이미지 추론 시간은 500ms 이내이어야 한다.<br>`REQ-004I-2` 손글씨 환경에서 문자 탐지 정확도는 검증 데이터셋 기준 85% 이상이어야 한다.<br>`REQ-004I-3` region_score 임계값은 기본 0.5이며 설정 파일로 조정 가능해야 한다.<br>`REQ-004I-4` 탐지된 Bounding Box 정보는 원본 이미지 좌표계 기준으로 반환되어야 한다.<br>`REQ-004I-5` 탐지 문자 수가 0인 경우 사용자에게 '글씨를 인식할 수 없습니다' 오류를 반환해야 한다. |
...
| **Action** | ① 탐지된 모든 문자 Bounding Box의 높이/너비를 수집한다. ② **크기 균일성 분석**: 전체 문자 높이의 평균과 표준편차를 산출하여 균일성 점수를 계산한다. 표준 서체 대비 각 문자의 크기 편차(%)를 산출한다. ③ **기울기 분석**: 각 문자 Bounding Box의 주축(Principal Axis) 각도를 OpenCV `minAreaRect`로 측정하여 기울기 각도(°)를 산출한다. 전체 문자의 기울기 일관성(표준편차)을 계산하여 일관성 점수를 산출한다. ④ **행 정렬 분석**: 동일 행으로 분류된 문자들의 기준선(baseline) 편차를 측정하여 정렬 점수를 산출한다. ⑤ 크기 균일성·기울기 일관성·행 정렬 점수를 가중 합산하여 종합 점수(0~100)를 산출한다. ⑥ 결과를 SFR-007과 PostgreSQL에 전달한다. |
